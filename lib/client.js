...
/** Storage keys for the panel's remembered geometry. */
const WIDTH_KEY = "dsh-live-diff:width";
const HEIGHT_KEY = "dsh-live-diff:height";
const LEFT_KEY = "dsh-live-diff:left";
const TOP_KEY = "dsh-live-diff:top";
/** Default / minimum / margin from the viewport's right edge, in px. */
const WIDTH_DEFAULT = 440;
const WIDTH_MIN = 280;
const WIDTH_MARGIN = 12;
/**
 * Default height as a fraction of the viewport.
 *
 * Not full height. The panel used to span `100vh - 24px`, which put a 440px-wide
 * column over the whole right edge — including the bottom-right corner where the
 * host keeps controls. It covered one of them outright. A panel that hides the
 * application's own buttons is worse than a shorter diff.
 */
const HEIGHT_FRACTION = 0.6;
/** Smallest useful height: below this the header and one row do not fit. */
const HEIGHT_MIN = 180;

/**
 * Read a remembered pixel value.
 *
 * `localStorage` can throw (disabled storage, a partitioned frame, a sandboxed
 * preview), and a layout preference is never worth breaking the panel over — so
 * every access is guarded and the fallback stands in on any failure.
 * @param key - storage key.
 * @param fallback - value to use when nothing usable is stored.
 * @param min - reject stored values below this (`NaN` and junk land here too).
 * @returns the stored value or the fallback.
 */
function storedPx(key, fallback, min) {
	try {
...
/** Remember a pixel value. Failures are ignored (see {@link storedPx}). */
function storePx(key, value) {
	try {
...
function storedWidth() {
	return storedPx(WIDTH_KEY, WIDTH_DEFAULT, WIDTH_MIN);
}

/** The height to open with: the last dragged value, else a fraction of the viewport. */
function storedHeight() {
	const viewport = document.documentElement === null ? 0 : document.documentElement.clientHeight;
	// `clientHeight` can be 0 or absent (a non-browser DOM, a detached document), and
	// `0 * 0.6` would then be a height of zero — a panel with nothing in it. Fall back
	// to the minimum rather than trusting the arithmetic.
	const fraction = Number.isFinite(viewport) && viewport > 0 ? Math.round(viewport * HEIGHT_FRACTION) : 0;
	return storedPx(HEIGHT_KEY, Math.max(HEIGHT_MIN, fraction), HEIGHT_MIN);
}

/** The left offset to open with, or null when the panel should stay right-anchored. */
function storedLeft() {
	return storedPx(LEFT_KEY, Number.NaN, 0);
}

/** The top offset to open with, or null when the panel is not positioned yet. */
function storedTop() {
	return storedPx(TOP_KEY, Number.NaN, 0);
}
...
function ensureOverlay() {
	...
	node.id = OVERLAY_ID;
	// Geometry is remembered: the panel is draggable, and a panel that jumps back
	// to covering the host's controls on every reload would be worse than one that
	// could not move at all.
	//
	// Anchoring switches from `right`/`bottom` to `left`/`top` as soon as the user
	// drags it, because a remembered position has to be measured from a corner that
	// does not move when the viewport resizes.
	const rememberedLeft = storedLeft();
	const rememberedTop = storedTop();
	node.style.position = "fixed";
	node.style.zIndex = "2147483647";
	node.style.width = `${String(storedWidth())}px`;
	node.style.height = `${String(storedHeight())}px`;
	if (Number.isFinite(rememberedLeft) && Number.isFinite(rememberedTop)) {
		node.style.left = `${String(rememberedLeft)}px`;
		node.style.top = `${String(rememberedTop)}px`;
	} else {
		node.style.right = `${String(WIDTH_MARGIN)}px`;
		node.style.bottom = `${String(WIDTH_MARGIN)}px`;
	}
	node.style.display = "flex";
	node.style.flexDirection = "column";
	node.style.color = "#e6edf3";
	node.style.borderRadius = "10px";
	node.style.overflow = "hidden";
	// Fixed stack, no theme variable: the panel sits outside the shell's theming,
	// and `var()` with no binding would fall back to the page default (a
	// proportional face at an unknown size) — unreadable for a diff.
	node.style.font = "12px/1.6 ui-monospace,SFMono-Regular,'Cascadia Mono','Cascadia Code',Menlo,Consolas,'DejaVu Sans Mono',monospace";
	...
	style.textContent = [
		...
		// Resize grips on all four corners. `position:absolute` so they are not flex
		// children (which would steal height from the list or width from the rows). The
		// grab area is larger than the mark it draws — a 3px target is not grabbable.
		//
		// The mark is an L-shaped border rotated to face outward from its own corner, so
		// each grip reads as belonging to that corner. `nwse-resize` / `nesw-resize`
		// match the diagonal each corner actually moves along.
		`#${OVERLAY_ID} .rz{position:absolute;width:18px;height:18px;z-index:3;display:flex;align-items:center;justify-content:center;touch-action:none}`,
		`#${OVERLAY_ID} .rz i{display:block;width:9px;height:9px;transition:border-color .12s ease}`,
		`#${OVERLAY_ID} .rz-tl{left:0;top:0;cursor:nwse-resize}`,
		`#${OVERLAY_ID} .rz-tl i{border-top:2px solid rgba(255,255,255,.28);border-left:2px solid rgba(255,255,255,.28);border-top-left-radius:3px}`,
		`#${OVERLAY_ID} .rz-tr{right:0;top:0;cursor:nesw-resize}`,
		`#${OVERLAY_ID} .rz-tr i{border-top:2px solid rgba(255,255,255,.28);border-right:2px solid rgba(255,255,255,.28);border-top-right-radius:3px}`,
		`#${OVERLAY_ID} .rz-bl{left:0;bottom:0;cursor:nesw-resize}`,
		`#${OVERLAY_ID} .rz-bl i{border-bottom:2px solid rgba(255,255,255,.28);border-left:2px solid rgba(255,255,255,.28);border-bottom-left-radius:3px}`,
		`#${OVERLAY_ID} .rz-br{right:0;bottom:0;cursor:nwse-resize}`,
		`#${OVERLAY_ID} .rz-br i{border-bottom:2px solid rgba(255,255,255,.28);border-right:2px solid rgba(255,255,255,.28);border-bottom-right-radius:3px}`,
		`#${OVERLAY_ID} .rz:hover i,#${OVERLAY_ID} .rz.on i{border-color:rgba(120,200,255,.9)}`,
		`#${OVERLAY_ID} .bar{cursor:grab}`,
		`#${OVERLAY_ID} .bar:active{cursor:grabbing}`,
		`#${OVERLAY_ID} .w{flex:none;color:#9a9a9a;font-size:11px;font-variant-numeric:tabular-nums;display:none}`,
		// Suppress text selection during a drag, and keep each handle's own cursor (a
		// blanket value would be wrong for the corners and the move handle).
		`body.dshld-resizing,body.dshld-resizing *{user-select:none !important}`,
		`body.dshld-resizing .rz-tl,body.dshld-resizing .rz-br{cursor:nwse-resize !important}`,
		`body.dshld-resizing .rz-tr,body.dshld-resizing .rz-bl{cursor:nesw-resize !important}`,
		`body.dshld-resizing .bar{cursor:grabbing !important}`
	].join("\n");
	...
	// Keep the handlers' disposer: their `window` listeners outlive the element, so
	// `startOverlay`'s teardown has to remove them explicitly.
	node.__detachResize = attachPanelDrag(node, bar, widthChip);
...
/**
 * Make the panel draggable: move it by its header, resize it by its left edge or
 * its bottom edge.
 *
 * The move handle is the header, and it exists because the panel has to be able to
 * get out of the way. It is anchored bottom-right at `z-index: 2147483647`, so at
 * its default size it covered the host's own controls in that corner — one of them
 * could not be clicked at all. Hiding the panel was the only escape, and that took
 * the diff away with it.
 *
 * All three handles share one mechanism. `pointerdown` records the start, `move`
 * computes a delta, `up` persists. Pointer capture is used because the strips are
 * only a few pixels wide and the pointer leaves them immediately; a `window`
 * `pointerup` listener backs it up for engines or situations where capture is
 * unavailable, since without either the drag would strand its "resizing" state.
 *
 * Position is only switched from right/bottom anchoring to left/top anchoring when
 * the user actually drags, so the default layout is unchanged and a remembered
 * position is measured from a corner that does not move when the viewport resizes.
 *
 * Both resize strips are created here rather than passed in. They were props once,
 * and the caller that forgot one produced a panel with no width handle at all — the
 * handles belong to the element they resize.
 * @param node - the overlay element.
 * @param bar - the header, used as the move handle.
 * @param chip - the bar's readout.
 * @returns a disposer for `startOverlay`.
 */
function attachPanelDrag(node, bar, chip) {
	/** The panel's current size, read from its own style rather than the rect.
	 *
	 * `getBoundingClientRect()` is 0 for an element that is not in the document, and
	 * this runs before the panel is appended — reading it there collapsed the panel
	 * to its minimum on the first window resize.
	 */
	const measured = (property, fallback) => {
		const parsed = Number.parseFloat(node.style[property]);
		return Number.isFinite(parsed) && parsed > 0 ? parsed : fallback;
	};

	let width = measured("width", storedWidth());
	let height = measured("height", storedHeight());

	/**
	 * Resize grips, one per corner.
	 *
	 * A single top-left grip replaced two edge strips (the width one was reported as
	 * "dragging the left handle changes the right side" — the panel is anchored by
	 * `right`, so widening it moves the left edge and pins the right one, which reads
	 * from outside as the wrong edge moving). Four corners are the conventional
	 * affordance and remove the guesswork about which edge a grip belongs to: the
	 * corner under the pointer is the corner that stays put, and the two opposite
	 * edges move.
	 *
	 * Each is `position:absolute` so it participates in neither flex direction; a flex
	 * child would steal height from the list or width from the rows. The class carries
	 * the corner so the CSS can place it and set the matching cursor.
	 *
	 * `widthDelta`/`heightDelta` are the signs that turn a pointer delta into a size
	 * change for that corner:
	 *
	 *   - dragging AWAY from the panel's own corner makes it bigger, so a grip on the
	 *     left grows on a negative `dx` and one on the right on a positive `dx`;
	 *   - likewise the top grows on a negative `dy` and the bottom on a positive `dy`.
	 */
	const corners = [
		{ name: "tl", cursor: "nwse-resize", widthDelta: -1, heightDelta: -1 },
		{ name: "tr", cursor: "nesw-resize", widthDelta: 1, heightDelta: -1 },
		{ name: "bl", cursor: "nesw-resize", widthDelta: -1, heightDelta: 1 },
		{ name: "br", cursor: "nwse-resize", widthDelta: 1, heightDelta: 1 }
	];
	const grips = corners.map((corner) => {
		const element = document.createElement("div");
		element.className = `rz rz-${corner.name}`;
		element.title = "拖动调整大小（双击恢复默认）";
	...
	const applyWidth = (value) => {
		width = Math.min(maxWidth(), Math.max(WIDTH_MIN, value));
		node.style.width = `${String(Math.round(width))}px`;
		chip.textContent = `${String(Math.round(width))} px`;
	};

	const applyHeight = (value) => {
		const viewport = document.documentElement === null ? 0 : document.documentElement.clientHeight;
		const ceiling = Math.max(HEIGHT_MIN, viewport - 2 * WIDTH_MARGIN);
		height = Math.min(ceiling, Math.max(HEIGHT_MIN, value));
		node.style.height = `${String(Math.round(height))}px`;
	};

	/**
	 * Switch to left/top anchoring, preserving what is on screen.
	 *
	 * Called on the first move of a drag that changes the panel's position. Uses the
	 * live rect, which is correct here because by then the panel is definitely in the
	 * document.
	 */
	const anchorToTopLeft = () => {
		// Truthiness, not `!== ""`. An unset style property reads as `""` in a browser
		// but as `undefined` in a plain DOM shim, and `undefined !== ""` is true — the
		// guard returned early, no anchor was written, and the caller captured `NaN`
		// as the starting position, so the panel jumped to `NaNpx`.
		if (node.style.left && node.style.top) return;
		const rect = node.getBoundingClientRect();
		node.style.left = `${String(Math.round(rect.left))}px`;
		node.style.top = `${String(Math.round(rect.top))}px`;
		node.style.right = "";
		node.style.bottom = "";
	};

	/**
	 * Clamp one axis, or `null` when the current value is not a number.
	 *
	 * `null` means "leave it alone": writing a fallback here would move a panel the
	 * caller never asked to move.
	 * @param value - the parsed style value.
	 * @param max - the largest allowed value.
	 * @returns the clamped value, or `null` to skip.
	 */
	...
	/** Keep the panel inside the viewport so it can never become unreachable. */
	const clampIntoView = () => {
		const viewportWidth = document.documentElement === null ? 0 : document.documentElement.clientWidth;
		const viewportHeight = document.documentElement === null ? 0 : document.documentElement.clientHeight;
		// `parseFloat` of unset/empty is `NaN`, and `Math.min`/`Math.max` propagate it —
		// `Math.max(0, NaN)` is `NaN`, so an unguarded clamp wrote the literal string
		// `NaNpx` and the panel vanished. Only reachable once a resize can call this
		// without the header having anchored the position first.
		const left = clampAxis(Number.parseFloat(node.style.left), viewportWidth - width);
		const top = clampAxis(Number.parseFloat(node.style.top), viewportHeight - height);
		if (left !== null) node.style.left = `${String(Math.round(left))}px`;
		if (top !== null) node.style.top = `${String(Math.round(top))}px`;
	};

	/**
	 * Wire one handle.
	 *
	 * `onBegin` runs on `pointerdown` and is where the handle captures the value it
	 * is about to adjust. That split matters: the pointer delta is measured from the
	 * press point, so applying it to a value that the previous `move` already
	 * updated double-counts the drag — the panel then moves half as far as the
	 * pointer on every step after the first.
	 * @param element - the handle.
	 * @param onBegin - captures the starting state.
	 * @param onMove - applies the delta, measured from the press point.
	 * @param onEnd - persists the result.
	 * @param onReset - double-click action.
	 * @returns a disposer for this handle.
	 */
	const wire = (element, onBegin, onMove, onEnd, onReset) => {
		let startX = 0;
		let startY = 0;
		let active = false;
		...
		const onDown = (event) => {
			// A drag must not start on a control.
			//
			// The move handle is the header, and the header contains the collapse and
			// hide buttons. Without this the press began a drag that called
			// `preventDefault()`, which suppressed the buttons' click — the hide button
			// stopped working entirely. The handle and its controls share one element,
			// so the drag has to yield to them.
			...
			try {
				...
				// The window listener above covers the pointer leaving the element.
			}
		...
		const onMoveEvent = (event) => {
			if (!active) return;
			onMove(event.clientX - startX, event.clientY - startY);
		};

		const onDouble = () => onReset();

		element.addEventListener("pointerdown", onDown);
		element.addEventListener("pointermove", onMoveEvent);
		...
		return () => {
			...
			element.removeEventListener("pointermove", onMoveEvent);
	...
	// Resize: each corner changes width and height together, with the sign pair that
	// matches its own corner (see the `corners` table above).
	//
	// The size change alone is not enough: the panel is laid out from its top-left
	// corner, so growing it moves the bottom-right and the dragged corner stays put —
	// exactly backwards, and reported as "dragging the top-left moves the bottom-right".
	// Each corner therefore also moves the two edges it owns by the size change that
	// was actually applied (`width - widthAtStart`, not the raw delta: the size is
	// clamped, and the edge has to follow the clamp or the corner under the pointer
	// drifts away from it at the limits).
	//
	// The position is only switched from right/bottom anchoring to left/top when a move
	// genuinely needs to, so a double-click reset — which changes no position — leaves
	// the default anchoring alone.
	let widthAtStart = width;
	let heightAtStart = height;
	let resizeLeftAtStart = "";
	let resizeTopAtStart = "";

	const detachResize = grips.map((corner) => wire(
		corner.element,
		() => {
			widthAtStart = width;
			heightAtStart = height;
			resizeLeftAtStart = node.style.left;
			resizeTopAtStart = node.style.top;
			// The default anchoring is right/bottom, which leaves no left/top for the move
			// step to offset — the panel would jump to the viewport origin on the first
			// resize instead of growing from its own corner. Anchoring on pointerdown
			// resolves the live rect once, while it is still correct; it is a no-op once
			// the position is absolute, so a later drag does not re-read the rect.
			anchorToTopLeft();
		},
		(dx, dy) => {
			applyWidth(widthAtStart + corner.widthDelta * dx);
			applyHeight(heightAtStart + corner.heightDelta * dy);
			if (width === widthAtStart && height === heightAtStart) return;
			// The panel is laid out from its top-left corner, so `left` is the left edge and
			// `top` the top edge, and the dragged corner's new position is its old position
			// plus how far it travelled.
			//
			// Its travel is `-widthDelta * dx` (and `-heightDelta * dy`). Both signs matter
			// and neither is guessable from the other: `widthDelta` is the sign that makes
			// "away from the panel" mean "bigger", i.e. the size is `widthDelta * dx`, while
			// the grip sits on the edge whose displacement is `-dx`. So a left-hand grip
			// (negative `widthDelta`) follows the pointer leftwards while the right edge stays
			// put, and a right-hand grip follows it rightwards with the left edge pinned.
			//
			// Each grip moves only the axes it owns: a left/right grip changes `left`, a
			// top/bottom grip changes `top`. Getting either half of this wrong is invisible in
			// the size assertions and shows up only as the wrong corner moving, which is how
			// this was reported — twice.
			const movesLeft = corner.widthDelta < 0;
			const movesTop = corner.heightDelta < 0;
			const startLeft = Number.parseFloat(resizeLeftAtStart) || 0;
			const startTop = Number.parseFloat(resizeTopAtStart) || 0;
			const offsetLeft = -corner.widthDelta * dx;
			const offsetTop = -corner.heightDelta * dy;
			node.style.left = `${String(Math.round(startLeft + (movesLeft ? offsetLeft : 0)))}px`;
			node.style.top = `${String(Math.round(startTop + (movesTop ? offsetTop : 0)))}px`;
		},
		() => {
			// A resize can push the panel past the viewport edge, and an off-screen panel
			// cannot be grabbed to bring back.
			clampIntoView();
			storePx(WIDTH_KEY, width);
			storePx(HEIGHT_KEY, height);
			storePx(LEFT_KEY, Number.parseFloat(node.style.left));
			storePx(TOP_KEY, Number.parseFloat(node.style.top));
		},
		() => {
	...
	// Position: the header. This is what lets the panel be moved off the host's
	// controls instead of having to be hidden.
	let leftAtStart = 0;
	let topAtStart = 0;
	const detachMove = wire(
		bar,
		() => {
			// Anchor first, then read: `anchorToTopLeft` is what turns the live rect
			// into left/top, and reading before it would measure against `right`.
			anchorToTopLeft();
			leftAtStart = Number.parseFloat(node.style.left);
			topAtStart = Number.parseFloat(node.style.top);
		},
		(dx, dy) => {
			node.style.left = `${String(Math.round(leftAtStart + dx))}px`;
			node.style.top = `${String(Math.round(topAtStart + dy))}px`;
		},
		() => {
			clampIntoView();
			storePx(LEFT_KEY, Number.parseFloat(node.style.left));
		...
		() => {
			// Back to the default corner. This is the way out if the panel was
			...
			node.style.bottom = `${String(WIDTH_MARGIN)}px`;
			try {
				window.localStorage.removeItem(LEFT_KEY);
				window.localStorage.removeItem(TOP_KEY);
			} catch {
				// Storage disabled: the in-memory reset above already took effect.
			}
		}
	);

	// The header carries the move handle, so give it the affordance.
	bar.style.cursor = "grab";
	bar.title = "拖动标题栏移动面板（双击回到默认位置）";

	// A viewport that shrinks could leave the panel overhanging or off-screen.
	const onWindowResize = () => {
		applyWidth(width);
		applyHeight(height);
		// Only re-clamp a panel that has actually been positioned. Truthiness, for the
		// same reason as in `anchorToTopLeft`: an unset `left` is `undefined` in a
		// shim, and clamping against `NaN` would snap the panel to the top-left corner
		// on any window resize.
		if (node.style.left) clampIntoView();
	};
	 ...
	 * Undo every listener.
	 *
	 * Worth returning rather than relying on garbage collection: the handles' own
	 * listeners die with the element, but the `window` ones outlive it, so a leaked
	 * `resize` listener accumulates one per plugin load — and the live-reload path
	 * loads this plugin on every source change.
	 * @returns a disposer for `startOverlay`.
	 */
	return () => {
		for (const detach of detachResize) detach();
		detachMove();
...
function apply(ctx) {
	...
	ctx.slots.inject("sidebar.right.pane.tab.title", () => ctx.slots.register({
		...
		return { apply, inject, LiveDiffCard, DiffBody, DiffBodyScroller, DiffsView, inFlightDiffs, diagnoseWindow, formatCounts, highlightLine, languageOf, pathOfRunningCall, diffRows, liveDiffOf, readJsonFields, alignDiff, jsx, buildDiffView, patchOverlayList, paintDiffElement, setPlaceholder, ensureOverlay, renderOverlay, attachPanelDrag, storedWidth, storedHeight, maxWidth, WIDTH_DEFAULT, WIDTH_MIN, HEIGHT_MIN, HEIGHT_FRACTION, OVERLAY_ID };
	}
});
