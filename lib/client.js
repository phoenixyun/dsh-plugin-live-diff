...
function attachPanelDrag(node, bar, chip) {
	 ...
	 *
	 * Called on the first move. Uses the live rect, which is correct here because by
	 * then the panel is definitely in the document.
	 */
	...
	const clampIntoView = () => {
		...
		const left = Math.min(Math.max(0, Number.parseFloat(node.style.left)), Math.max(0, viewportWidth - width));
		const top = Math.min(Math.max(0, Number.parseFloat(node.style.top)), Math.max(0, viewportHeight - height));
