...
function paintRow(element, row, language, force) {
	...
	// running animation, so skip when nothing about the text changed.
	if (force || text.__text !== row.text) {
...
function setPlaceholder(list, text) {
	...
	list.__placeholder.textContent = text;
}
...
function ensureOverlay() {
	...
	attachResizeHandle(node, widthChip);
...
function attachResizeHandle(node, chip) {
	...
	let startX = 0;
	let startWidth = 0;
	let current = node.getBoundingClientRect().width;
	...
	strip.addEventListener("pointerdown", (event) => {
		...
		try {
			...
			// Pointer capture is a nicety; without it the drag still works as long
...
function finishedViews() {
	return overlayLastDiffs.map((diff) => {
		...
		for (const row of view.rows) row.caret = false;
		return view;
	});
}
...
function startOverlay(ctx) {
	...
	const tick = () => {
		...
		try {
			...
			try {
				const num = (value) => (value === null || value === void 0 ? "-" : String(value));
				document.title = `[LD e${String(entries.length)} win${num(report.entryCount)} tr${num(report.transientCount)} dl${num(report.deltaCount)} acc${num(report.accumulating)}] ${report.reason}`;
			} catch {
 ...
 * The primitive renders each row as plain text, so the caret rides on the last
 * added line's content rather than being its own animated element.
 * @param props - diff material plus the scroller wiring from {@link useFollowScroll}.
 * @returns the body element.
 */
function DiffBody({ diff, cwd, scrollerRef, onScroll }) {
	const { rows, added, removed } = diffRows(diff);
	const caretIndex = rows.reduce((found, row, index) => (row.kind === "add" ? index : found), -1);
	const liveText = diff.streaming && caretIndex >= 0
		? rows.map((row, index) => (index === caretIndex ? `${row.text}\u258f` : row.text)).join("\n")
		: diff.newText;
	const liveOldText = diff.streaming
		? rows.filter((row) => row.kind === "remove").map((row) => row.text).join("\n")
		: diff.oldText;
	...
	return jsx("div", { className: "dshld_root" }, [
		...
		jsx("div", { className: "dshld_body", key: "body", ref: scrollerRef, onScroll }, jsx(DiffBlock, {
			diffs: [{
...
function LiveDiffCard({ block, toolName, cwd }) {
	...
	return jsx(DiffBody, { diff, cwd, scrollerRef: scroll.scrollerRef, onScroll: scroll.onScroll });
}
...
function DiffsView(props) {
	...
	// One scroller for the whole page: the hook runs once, and a long edit keeps
	// the newest line in view no matter how many diffs are listed.
	const scroll = useFollowScroll(entries.length === 1 ? entries[0].diff : null);
	...
	useEffect(() => {
		...
		// Also write the reading into the document title.
		//
		// Everything above assumes the panel is visible. It was not: the view
		// reported faithfully while the sidebar painted nothing, which made the
		// channel's own output unreadable. The title bar sits outside every theme,
		// layout and stacking rule, so it still shows the reading when the surface
		// itself cannot be seen.
		try {
			const num = (value) => (value === null || value === void 0 ? "-" : String(value));
			const parts = [
				`e${String(entries.length)}`,
				`win${num(report === null ? null : report.entryCount)}`,
				`tr${num(report === null ? null : report.transientCount)}`,
				`dl${num(report === null ? null : report.deltaCount)}`,
				`acc${num(report === null ? null : report.accumulating)}`
			];
			document.title = `[LD ${parts.join(" ")}] ${report === null ? "no report" : report.reason}`;
		} catch {
			// A diagnostic must never be the reason a surface fails.
		}
	...
	//     component then renders perfectly, sets `document.title`, beacons
	//     faithfully — and paints nothing, which is indistinguishable from "no
	//     data". The fix is to state no height at all and let the block lay itself
	//     out from its content.
...
function apply(ctx) {
	...
	ctx.slots.inject("sidebar.right.pane.tab.title", () => ctx.slots.register({
		...
		return { apply, inject, LiveDiffCard, DiffBody, DiffsView, inFlightDiffs, diagnoseWindow, formatCounts, highlightLine, languageOf, pathOfRunningCall, diffRows, liveDiffOf, readJsonFields, alignDiff, jsx, buildDiffView, patchOverlayList, paintDiffElement, setPlaceholder, ensureOverlay, renderOverlay, attachResizeHandle, storedWidth, maxWidth, WIDTH_DEFAULT, WIDTH_MIN, OVERLAY_ID };
	}
});
