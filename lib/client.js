...
function ensureOverlay() {
	...
	style.textContent = [
		...
		// Resize grip, on the top-left corner. `position:absolute` so it is not a flex
		// child (which would steal height from the list or width from the rows). It
		// beats the list's stacking by `z-index`, and it is deliberately larger than
		// the dot it draws — a 3px target is not grabbable.
		//
		// The visual is a small diagonal, which is the conventional "resize" mark and,
		// unlike a single edge line, says that both dimensions change.
		`#${OVERLAY_ID} .rz{position:absolute;left:0;top:0;width:18px;height:18px;cursor:nwse-resize;z-index:3;display:flex;align-items:center;justify-content:center;touch-action:none}`,
		`#${OVERLAY_ID} .rz i{display:block;width:9px;height:9px;border-top:2px solid rgba(255,255,255,.28);border-left:2px solid rgba(255,255,255,.28);border-top-left-radius:3px;transition:border-color .12s ease}`,
		...
		// blanket `ew-resize` would be wrong for the corner and the move handle).
		`body.dshld-resizing,body.dshld-resizing *{user-select:none !important}`,
		`body.dshld-resizing .rz{cursor:nwse-resize !important}`,
...
function attachPanelDrag(node, bar, chip) {
	...
	/**
	 * One resize grip, on the top-left corner.
	 *
	 * It was two edge strips (left and bottom), and the width one was reported as
	 * "dragging the left handle changes the right side": the panel is anchored by
	 * `right`, so widening it moves the left edge and leaves the right edge pinned —
	 * which from the outside looks like the *other* side is what changed. A corner
	 * grip removes the ambiguity: it visibly moves both the edge under the pointer and
	 * the size, and it adjusts width and height in one gesture instead of making the
	 * user find two separate 6px strips.
	 *
	 * It is `position:absolute` so it participates in neither flex direction; a flex
	 * child would steal height from the list or width from the rows.
	 */
	const grip = document.createElement("div");
	grip.className = "rz";
	grip.title = "拖动调整大小（双击恢复默认）";
	...
	// Resize: the corner grip changes width and height together. Dragging left widens
	// the panel (it is right-anchored, so the left edge is the one that moves out) and
	// dragging up makes it taller (it is bottom-anchored).
	let widthAtStart = width;
	let heightAtStart = height;
	const detachResize = wire(
		grip,
		() => {
			widthAtStart = width;
		...
		(dx, dy) => {
			applyWidth(widthAtStart - dx);
			applyHeight(heightAtStart - dy);
		},
		() => {
