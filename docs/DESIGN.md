...
> **This is a decision log, not a manual.** It was written in the order the problems
> were hit, so it argues positions that were later revised and describes surfaces
> that were later abandoned. Read it for the reasoning, not for the current shape of
> the plugin — for that, see [`../README.md`](../README.md).
>
> Two sections are now **out of date**, and are kept only because the dead end is
> itself the useful part:
>
> - **"Where the live diff is rendered"** lists the right-sidebar tab as a live
>   surface. It is not. The sidebar pane mounted, ran its effects, beacons fired —
>   and painted nothing, because the host does not paint a sidebar tab body. The
>   product surface is now a plain DOM panel appended to `document.body`. The tab
>   registration is still there and still must stay (removing it changes nothing
>   visible; keeping it costs nothing), but nothing renders through it.
> - **"What this plugin does"** says rendering goes through DSH's `DiffBlock`. That
>   is true only of the **chat transcript card**. The floating panel draws its own
>   rows, because it has to append to a row that is still growing and `DiffBlock`
>   takes a finished line list.
>
> What did **not** change, and is still the load-bearing reasoning here: the partial
> JSON reader, the pin-to-newest-line rule, the layout traps, and the measured
> transport granularity.

## Where the live diff is rendered, and why

Three surfaces now, one shared reader:

| Surface | Registration | When it renders |
| --- | --- | --- |
| Chat transcript row | `tool.call.toolview` (keyed `edit` / `write`) | **Only after the call settles** |
| Right-sidebar **Live Diffs** tab | `sidebarRightTabs` + `sidebar.right.pane.tab` | Registered, but **paints nothing** — see the note above |
| Floating panel (`#dsh-live-diff-overlay`) | none; plain DOM on `document.body` | Continuously, while the call runs — **this is the product surface** |
...
through DSH's own `DiffBlock` primitive **in the chat transcript**.

> The floating panel was added later and draws its own rows instead: `DiffBlock`
> takes a finished line list, and the panel's whole point is to append to a line that
> is still growing.

The shared reader and the derived rows work the same way on both surfaces:
